...
const EventOrganized = () => {
  ...
  const downloadCSV = React.useCallback((array) => {
    ...
    if (csv == null) return;
    const filename = 'institute_event_organized_export.csv';
...
export const eventOrganisedColumns = [
  ...
  },
  {
    ...
    cell: row => {
      ...
      if (typeof row.category === 'object' && !Array.isArray(row.category)) {
        return Object.values(row.category).filter(v => v && typeof v === 'string').join('');
      }
